\section{Actual Battery}
    ...
    \subsection{Temperature Behavior of Battery}
        ...
        The Samsung INR21700 30T 3Ah Li-ion Battery Dataset~\cite{samsung_dataset} contains experimental data at various temperatures. Using this dataset, we perform a 
        ...
        If we consider the temperature effect, the simulation heatmap of Time to Empty from $100\%$ $SOC$ under different power load and temperature will 
        be as follows: 
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.6\linewidth]{depletion_time_heatmap.png}
            \caption{Heatmap of Time to Empty with Power Load and Temperature}
            \label{fig:simple_time_to_empty}
        \end{figure}
    ...
    \subsection{Battery in Different Work Loads}
        ...
        subsystems~\cite{smartphone_power}. According to the \textbf{Principle of Power Superposition}, when individual modules share a power supply but operate relatively 
        ...
        By utilizing acquired smartphone state data~\cite{android_dataset}, we estimate the parameters within these models, thereby obtaining a comprehensive power consumption 
        model for complex usage scenarios.
        ...
        Furthermore, modern ARM processors commonly adopt a \textbf{big.LITTLE heterogeneous architecture}~\cite{big_little}, featuring separate big and small cores with 
        ...
        According to the Friis transmission equation~\cite{friis}, received power is inversely proportional to the square of the distance:
        \begin{equation}
        ...
        Results demonstrate that the white-box power model constructed based on the real-world smartphone dataset achieves satisfactory prediction performance, with an $R^2$ of 0.606, an MAE of 0.355 W, and an RMSE of 0.461 W. Among the factors analyzed, CPU usage and data network activity exhibit the most significant impact on power draw, whereas GPS consumption and the energy-saving effects of the two included modes show relatively minor influence. As mentioned above, the energy-saving effects of these two modes in practice primarily stem from their influence on the aforementioned features; the small coefficients here reflect that their impact on factors beyond those already captured is relatively minor.
    ...
    \subsection{Heat Transfer Model in High-Performance Scenarios}
        ...
        if the battery temperature exceeds $50\mathrm{^\circ C}$, the device will shutdown to protect the battery~\cite{tran2022_processes}. In reality, smartphones often decreases processor
        ...
        \begin{figure}[H]
            ...
            \label{fig:complex_time_to_empty}
        \end{figure}
    ...
    \subsection{Answer to Problem 1}
        In section \ref{sec:ideal_battery}, we have already built an simplistic ideal battery model. Then in this section, we have considered more complex
        factors that affect real battery performance, which should answer Problem 1.

    \subsection{Answer to Problem 2}
        Heatmap \ref{fig:simple_time_to_empty} and \ref{fig:complex_time_to_empty} show the heatmaps of discharge time under different power load and 
        temperature. From these two figures we can see that in low power load scenarios, temperature has a significant impact on battery performance. 
        However, as power load increases, the influence of temperature gradually diminishes. In high power load scenarios, the battery performance is 
        primarily determined by the power load itself, with temperature playing a secondary role.