\section{Actual Battery}
    ...
    \subsection{Battery in Different Work Loads}
        ...
        Subsequently, we can analyze the contribution rate of each parameter and evaluate the model's performance.

        %TODO: text font change

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.9\linewidth]{power_model_fitting.png}
            \caption{Model fitting results and parameter contribution analysis}
            \label{fig:power_model_fitting}
        \end{figure}

        \subsubsection{Usage Scenario Simulation}
        The model allows us to estimate the smartphone's power consumption under various typical usage scenarios.

        %TODO: replace figure with table

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.9\linewidth]{usage_scenarios_table.png}
            \caption{Power consumption under different usage scenarios}
            \label{fig:usage_scenarios}
        \end{figure}
