\section{Ideal Battery}
    ...
    \subsection{Thevenin Model}
        ...
        an equivalent circuit model is often used to represent the battery behavior~\cite{ecm_model}. The Thevenin model is a widely used equivalent circuit model that 
        ...
        However, if the battery is in an open-circuit state, the structure of the circuit will change:

        %TODO: open-circuit figure

        In that case, since $I = 0$, there is no voltage drop across $R_0$, so the terminal voltage $U_t$ is expressed by:
        \begin{equation}
