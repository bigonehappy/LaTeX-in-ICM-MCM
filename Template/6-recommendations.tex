...
Battery capacity degrades over charge-discharge cycles~\cite{battery_aging}. We adopt a simplified linear aging model:
\begin{equation}
...
Using the NASA Li-ion Battery Aging Dataset~\cite{nasa_dataset}, we fitted $\alpha = 0.000411$ per cycle ($R^2 = 0.97$), indicating 0.041\% capacity loss per cycle. At 500 cycles, capacity retention is approximately 79.5\%, consistent with the industry standard of 80\% end-of-life threshold.
...
\section{Further Discussions}
    \subsection{Strength}
        \begin{itemize}
            ...
            \item The model effectively captures the short-term relationship between smartphone power consumption, battery performance, and environmental
    ...
    \subsection{Future Work}
        \begin{itemize}
            \item \textbf{Consider the temperature distribution in the phone}. The current model assumes a uniform temperature for easy. Actually, the 
                  ...
                  itself generates may be not such important, especially in heavy usage scenarios.
